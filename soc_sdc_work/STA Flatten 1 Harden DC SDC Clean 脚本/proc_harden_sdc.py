...
def classify_command(command, config, pass1, unit_state):
    ...
    if cmd in set(["set_max_delay", "set_min_delay"]) and contains_get_ports(text):
        salvaged = salvage_single_boundary_delay(command, config, pass1)
    ...
    if cmd in DEFAULT_MODIFY_COMMANDS:
        if config.strict and command_involves_boundary_port_exception(cmd, text):
...
def classify_oversize_command(command, config, pass1, cmd):
    ...
    if cmd in set(["set_max_delay", "set_min_delay"]) and contains_get_ports(text):
        result = Result(command, "REMOVE", "oversize_boundary_delay_requires_manual_review", cmd, review_required=True)
        result.notes.append("Oversize delay command with get_ports was not shallow-mapped; review SoC boundary timing intent.")
        return result
...
def salvage_single_boundary_delay(command, config, pass1):
    text = command.normalized_text
    tokens = tokenize_tcl_words(text)
    endpoint_options = {}
    index = 1
    ...
    if from_boundary and not to_boundary and to_internal:
        reason = "boundary_from_mapped_keep_path"
        side = "from"
    elif to_boundary and not from_boundary and from_internal:
...
def command_involves_boundary_port_exception(cmd, text):
    return cmd in BOUNDARY_OWNED_COMMANDS and contains_get_ports(text)
...
def has_processed_signature(raw_text):
    signatures = [
        "Auto-generated SoC-callable harden SDC",
        "Generated by        : proc_harden_sdc.py",
        "Generated by: proc_harden_sdc.py",
        "SDC_PROCESS_VERSION",
        "SOC_STRIP(",
    ]
    return any(sig in raw_text for sig in signatures)
...
def main(argv=None):
    ...
    if has_processed_signature(raw_text) and not config.force_reprocess:
        ...
        print("ERROR: input appears to be already processed; use --force-reprocess if intentional.", file=sys.stderr)
        return 2
