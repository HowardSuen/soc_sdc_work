#!/usr/bin/env python3
"""Regression smoke tests for run_stage1_clean_sdc.py.
...
TOOL = os.path.abspath(os.path.join(HERE, "..", "run_stage1_clean_sdc.py"))
...
def test_boundary_multicycle_path_mapped():
    sdc = """\
set_multicycle_path 2 -setup -from [get_ports A] -to [get_pins u_dst/D]
set_multicycle_path 1 -hold -from [get_pins u_src/Q] -to [get_ports B]
set_multicycle_path 3 -setup -through [list [get_ports C] [get_pins u_mid/Q]] -to [get_pins u_dst2/D]
"""
    result = run_tool("boundary_multicycle_path_mapped", sdc)
    if result["code"] != 0:
        raise AssertionError("boundary_multicycle_path_mapped failed")
    assert_contains(result["out"], "# !!! REVIEW_REQUIRED COMMANDS BEGIN !!!")
    assert_contains(result["out"], "set_multicycle_path 2 -setup -from [get_pins u_soc/u_avfs/u_awm3_0/A] -to [get_pins u_soc/u_avfs/u_awm3_0/u_dst/D]")
    assert_contains(result["out"], "set_multicycle_path 1 -hold -from [get_pins u_soc/u_avfs/u_awm3_0/u_src/Q] -to [get_pins u_soc/u_avfs/u_awm3_0/B]")
    assert_contains(result["out"], "set_multicycle_path 3 -setup -through [list [get_pins u_soc/u_avfs/u_awm3_0/C] [get_pins u_soc/u_avfs/u_awm3_0/u_mid/Q]] -to [get_pins u_soc/u_avfs/u_awm3_0/u_dst2/D]")
    assert_contains(result["out"], "Boundary port exception was mapped get_ports -> get_pins; timing semantics must be reviewed.")
    assert_command_lines_not_contains(result["out"], "get_ports")
    assert_not_contains(result["removed"], "set_multicycle_path")
    assert_contains(result["modified"], "get_ports_mapped_to_get_pins")


def test_boundary_delay_mapping():
    sdc = """\
...
"""
    result = run_tool("boundary_delay_mapping", sdc)
    if result["code"] != 0:
        raise AssertionError("boundary_delay_mapping failed")
    ...
    assert_contains(result["out"], "set_max_delay 0.7 -from [get_pins u_soc/u_avfs/u_awm3_0/A] -to [get_pins u_soc/u_avfs/u_awm3_0/B]")
    ...
    assert_contains(result["out"], "# REVIEW_REQUIRED command_id=000003")
    ...
    assert_contains(result["out"], "Boundary from/to side was mapped get_ports -> get_pins with instance hierarchy for set_max_delay")
    ...
    assert_contains(result["modified"], "boundary_from_to_mapped_keep_path")
    assert_not_contains(result["removed"], "set_max_delay")
    assert_contains(result["removed"], "boundary_exception_owned_by_30")
...
def test_stage2_flatten_header_allowed():
    sdc = """\
# ============================================================
...
#   Version : v0.7.8
# ============================================================
# SDC_PROCESS_VERSION : v0.7.8
# sample_flatten.sdc generated by run_stage2_merge_delay.tcl
# This file is a flattened Stage 2 final SDC for the current integration scope.
set_max_delay 0.8 -from [get_ports A] -to [get_ports B]

# Embedded harden clean SDC audit header; this must not classify the whole
# Stage2 flatten file as a Stage1 output.
# Auto-generated SoC-callable harden SDC
# Generated by        : run_stage1_clean_sdc.py
# SDC_PROCESS_VERSION : v2.3.1
"""
    result = run_tool("stage2_flatten_header_allowed", sdc)
    if result["code"] != 0:
        raise AssertionError("Stage2 flatten SDC must not be rejected as a Stage1 output\nstdout=%s\nstderr=%s" % (
            result["stdout"],
            result["stderr"],
        ))
    assert_contains(result["out"], "set_max_delay 0.8 -from [get_pins u_soc/u_avfs/u_awm3_0/A] -to [get_pins u_soc/u_avfs/u_awm3_0/B]")


def test_stage1_generated_input_rejected():
    sdc = """\
# Auto-generated SoC-callable harden SDC
# Generated by        : run_stage1_clean_sdc.py
# SDC_PROCESS_VERSION : v2.3.1
set_false_path -from [get_pins u_a/Q] -to [get_pins u_b/D]
"""
    result = run_tool("stage1_generated_input_rejected", sdc)
    if result["code"] == 0:
        raise AssertionError("Stage1-generated input should be rejected")
    if "input appears to be a Stage1-generated file" not in result["stderr"]:
        raise AssertionError("Expected Stage1-specific idempotency error\nstderr=%s" % result["stderr"])
    assert_contains(result["report"], "[IDEMPOTENCY_ERROR]")
    assert_contains(result["report"], "Matched marker: Auto-generated SoC-callable harden SDC")
...
def main():
    ...
    tests = [
        ...
        test_stage2_flatten_header_allowed,
        test_stage1_generated_input_rejected,
    ]
    for test in tests:
        test()
