...
### 2.4.5 `common/harden_cleaned/<inst>_soc.sdc`

该文件由 harden DC/full flattened SDC 清洗生成，是全片 STA source 的一部分，用于保留 SoC STA 仍需要的 harden instance 内部约束。它和 01~30 并行使用：01~30 负责 SoC 级 clock、IO、interface、feedthrough、path exception；cleaned harden SDC 只负责对应 harden instance 内部约束。

可以保留：

- harden 内部 `set_disable_timing`，例如 SRAM/macro 内部 timing arc disable，前提是对象在 SoC STA netlist 中可见。
- harden 内部 `set_false_path` / `set_multicycle_path` / `set_max_delay` / `set_min_delay`，前提是不直接作用于 harden boundary `get_ports`，且对象映射到 `<inst>/...` 后仍是 harden 内部 scope。
- 单边 boundary delay salvage：`set_max_delay` / `set_min_delay` 若一侧是 harden boundary `get_ports`、另一侧是 harden 内部对象，可以将 boundary `get_ports` 映射为 SoC scope 下的 harden instance `get_pins <inst>/<port>`，内部对象同步上升 hierarchy，并保留整条 path 约束，例如 `-from get_ports -to get_pins` 转为 `-from get_pins <inst>/<port> -to get_pins <inst>/...`。该保留必须标为 review_required，用于保留 harden boundary pin 到 harden internal endpoint 的原 path 语义；仍需人工确认其是否属于 cleaned harden SDC，而不是 SoC interface timing。
- harden 内部 generated/local/virtual clock definition，前提是不与 01 中 SoC-visible clock creation 重复，并且所有保留 clock definition 已按 `<instance_prefix>_<old_clock_name>` 改名、通过 report review。即使该 clock 暂时未被最终 cleaned SDC 中其它 `get_clocks` 约束引用，也不因 unused 自动删除。

必须移除或转交：

- `create_clock` on harden boundary `get_ports`：归 01。
- target 为 harden boundary `get_ports` 的 SoC-visible generated/output clock：归 01。
- `set_input_delay` / `set_output_delay`：归 20 候选或 04 pad environment，不直接搬到 cleaned harden SDC。
- `set_clock_groups`：归 03。
- clock timing budget、derate、RC/back annotation：归 02/MMMC/flow。
- 直接涉及 harden boundary `get_ports` 的 `set_case_analysis`：归 scenario pre-setup。
- 直接涉及 harden boundary `get_ports` 且无法按单边 boundary delay salvage 安全映射的 `set_false_path` / `set_multicycle_path` / `set_max_delay` / `set_min_delay`：归 10/20/30 review 或 scenario exception，不能直接保留在 cleaned harden SDC。

cleaned harden SDC 的 removed/report 文件必须把这些被转交的 boundary 约束列为 review item，便于 10/20/30 或 scenario 表单继续接收。

实现要求：脚本应在 Tcl 解析前丢弃整行注释并保留行号，避免 DC 输出的大型注释约束影响运行时间。对超长有效命令，应采用浅层对象映射 guard：只对明确的 `[get_pins ...]`、`[get_ports ...]` 等简单 bracket get command，以及 safe `[list [get_pins ...] ...]` wrapper 增加 instance hierarchy；遇到变量、通配、`get_clocks`、非 safe 嵌套 collection 或复杂 option 时转入 `unsupported.sdc`，不得为了性能做不安全字符串替换。

装配建议：cleaned harden SDC 应在 clock creation、clock timing、clock relationship 之后 source，并在 10/20/30 之前或同阶段前置 source。它不得创建 SoC-visible clock，也不得覆盖 01~30 的 ownership。
...
Stage 5.5: cleaned harden internal constraints
  common/harden_cleaned/<inst>_soc.sdc
...
# Stage 5.5: cleaned harden internal constraints
source common/harden_cleaned/u_harden_a_soc.sdc
source common/harden_cleaned/u_harden_b_soc.sdc
