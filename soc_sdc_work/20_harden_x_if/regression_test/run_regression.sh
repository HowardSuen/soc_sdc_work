#!/usr/bin/env bash
# One-shot regression for 20_extract_harden_x_if.py
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/soc_sdc_20_regression.XXXXXX")"
trap 'rm -rf "$WORK_DIR"' EXIT

PYTHON_BIN="${PYTHON_BIN:-python3}"
EXTRACT_SCRIPT="$SCRIPT_DIR/../20_extract_harden_x_if.py"
...
grep -q "set_max_delay 1.2 -datapath_only -from \\[get_pins {u_a/data_o}\\] -to \\[get_pins {u_b/data_i}\\]" common/20_harden_x_if.sdc
...
grep -q "clock_relation=asynchronous blocks normal 20 budget" harden_x_if_check_report_common_all_all.txt

echo "20_harden_x_if regression passed in $WORK_DIR"
