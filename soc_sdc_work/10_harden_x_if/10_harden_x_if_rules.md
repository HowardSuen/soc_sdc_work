# 10_harden_x_if.sdc 规则说明

本文定义 `common/10_harden_x_if.sdc` 的职责、输入来源、channel 归并方法、表单字段和生成检查规则。

## 1. 目标

`10_harden_x_if.sdc` 用来表达 SoC 视角下 harden/subsys 边界的普通 interface timing budget。

10 的判断依据是约束意图，不是 SDC 命令名字：

- 普通 harden/subsys interface budget 归 10。
- 改变默认 STA 分析语义的 exception 归 20。
- SoC top pad 的外部 IO 环境归 04。

第一版 10 的最终 SDC 只生成 reviewed `set_max_delay` / `set_min_delay`，不直接生成 instance pin 上的 `set_input_delay` / `set_output_delay`。

需要特别注意：10 生成的 `set_max_delay` / `set_min_delay` 是 SoC boundary-to-boundary channel datapath budget，约束的是两个 SoC 级 endpoint 之间的 datapath，例如 top-level interconnect / glue logic / wrapper 组合逻辑。它不是 block 级 clock-relative `set_input_delay` / `set_output_delay` 的语义等价替换。
...
10 不是逐条 SDC 命令转换，而是按 interface channel 归并后生成。

一个 channel 表示 SoC 内部一条明确的连接关系：

```text
src instance/output port -> dst instance/input port
```
...
### 2.2 channel 端点来自集成表单

harden input 的 `-from` 端点应优先从集成表单推断。

harden output 的 `-to` 端点也应优先从集成表单推断。

脚本不应只根据单个 harden SDC 中的 `get_ports` 目标端口孤立生成约束；必须结合 SoC 集成表单确认真实连接端点。
...
因此 10 必须为每条 channel 明确 `budget_model`。
...
集成表单用于建立 SoC 连接关系，包括：

- SoC 下有多少个 harden/subsys。
- 每个 harden/subsys 的 instance name。
- 每个 input port 来源于哪个 instance/output port 或 SoC top pad。
- 每个 output port 去向哪些 instance/input port 或 SoC top pad。
- 一个 harden 可能被例化多次，所有规则按 instance 生效。

10 脚本必须先根据集成表单建立 channel inventory，再把 SDC 候选约束归并到 channel 上。

### 3.2 harden/subsys SDC

10 主要提取 harden/subsys SDC 中的非 pad interface delay：
...
10 需要引用 01 的 clock inventory 做辅助检查：
...
10 的 budget 推导可能依赖 clock period、uncertainty、latency、stage/corner 策略，这些信息归 02 管理。10 不应在脚本内部私自散落 hard-coded uncertainty 或 corner 假设；若 `converted_max` 不是直接来自 `interconnect_budget` 约定，必须在 `budget_basis` 中说明其与 01/02 clock setup 的关系。

10 需要与 03 的 clock relationship 信息保持一致。若 channel 两端 clock 在当前 assembled view 中被声明为 async / logically exclusive / physically exclusive，则该 channel 默认不能生成普通同步 interface budget；应转 20 或由 reviewer 明确给出 override basis。

第一版脚本先在 `interface_budget.clock_relation` 中保留人工/review 入口，并据此阻断 async/exclusive channel 的普通 10 budget 生成。后续可再扩展为自动读取 03 assembled view 并注入该字段。

## 4. 与其它 SDC 的边界

### 4.1 与 04_soc_io_pads.sdc

如果 harden/subsys port 连接到 SoC top pad，该约束不进 10。

- pad input/output delay、load、driving cell、input transition 归 04。
- 10 只处理 SoC 内部 harden/subsys interface channel。

### 4.2 与 20_harden_to_harden_exception.sdc

普通 interface budget 归 10。

exception 性质的路径归 20，例如：

- false path。
- multicycle path。
- async handshake path。
- configuration path。
- reset/test/mode-specific override。
- 为了偏离普通同步接口预算而设置的 `set_max_delay` / `set_min_delay` override。

### 4.3 与 30_feedthrough.sdc

如果路径是结构性 feedthrough，例如 harden 内部不锁存、只从 input 纯穿到 output，则优先归 30。

10 只表达普通 harden/subsys 边界 timing budget。

### 4.4 与 module/block signoff SDC

block 自身 signoff 时，`set_input_delay` / `set_output_delay` 仍然可以作用于 block top port。

SoC top 使用 10 时，不直接把这些命令改成：
...
如果 harden/subsys 以可见 wrapper/netlist 方式集成，且边界两端寄存器、clock path 和 timing arc 都可被 SoC STA 正常看到，那么普通 synchronous path 应优先由常规 clock STA 分析。此时 10 默认不再额外叠加 `set_max_delay -datapath_only`，否则可能与周期关系、skew/uncertainty 建模重复或冲突。

10 的 channel budget 主要适用于以下情况：
...
表单中应记录 `timing_model` 或 `budget_required`，用于说明为什么该 channel 需要 10 budget。
...
其中 `top_pad_to_harden` / `harden_to_top_pad` 归 04；`clock_connection` 归 01/02/03；`feedthrough` 归 30；`exception_path` 归 20。
...
对于 `fabric_to_harden` / `harden_to_fabric`，如果集成表单不能给出具体 fabric endpoint，脚本应保留 channel 候选但不自动生成 SDC；需要人工补齐 endpoint 或改用项目约定的 endpoint collection。
...
则 10 先记录为 destination input delay 候选。只有当该原始 delay 被确认是 `interconnect_budget`，或 reviewer 设置 `budget_model = manual_budget` 并给出 `derivation_basis`（例如 `period_based_budget`）后，才能形成 10 SDC 候选：
...
则 10 先记录为 source output delay 候选。只有当该原始 delay 被确认是 `interconnect_budget`，或 reviewer 设置 `budget_model = manual_budget` 并给出 `derivation_basis`（例如 `period_based_budget`）后，才能形成 10 SDC 候选：
...
- `-add_delay`。
- 同一 port 多 clock。
- source-synchronous / DDR 接口。pad 侧 source-sync 约束归 04；harden/subsys 内部 interface source-sync 留到后续 10 扩展。
...
10 的 interface budget 是时序数值，可能随 stage/corner/view 变化。第一版表单必须保留 `stage` / `corner` 字段，生成时不能把多个 corner 的数值平铺进同一个 SDC。
...
```text
common/10_harden_x_if_<stage>_<corner>.sdc
```

例如：

```text
common/10_harden_x_if_prects_ss_125.sdc
common/10_harden_x_if_postcts_ff_m40.sdc
```

同一个 view 内，所有 common harden/subsys interface channel budget 仍归集到一个 10 文件。

### 7.1 assembled view

10 的检查和生成按 assembled view 进行。assembled view 是当前生成目标下实际会被 source 到同一个 analysis view 的 10 约束集合：

```text
view-independent common 10
+ 当前 stage/corner 的 common 10 view-specific 行
+ 当前 scenario 的 10 overlay 行
+ 当前 scenario/stage/corner 的 10 view-specific overlay 行
```
...
10 文件在装配顺序中位于 04 之后、20 之前。20 可以在最后覆盖 exception 性质的路径语义，但不应覆盖普通 10 budget 来清 timing。
...
- `is_pad_related` / `is_clock_related` / `is_feedthrough`：从 channel inventory 带入 budget 行，用于阻断错误归属；这些行不应生成 10。
- `clock_relation`：记录该 channel 在 03/人工 review 中的关系，例如 `sync`、`async`、`logically_exclusive`、`physically_exclusive`；async/exclusive 默认阻断普通 10 budget。
...
- `datapath_only`：说明是否生成 `-datapath_only`，以及该工具是否支持该语义。
- `min_sign_review`：说明 min 值是否完成 sign/hold 语义 review。
- `budget_basis`：必须说明 budget 来源，例如 block signoff assumption、架构预算、接口协议、人工裁决。
- `relationship_override_basis`：当 03 中已有 async/exclusive 关系但仍要求生成 10 budget 时，必须说明裁决依据；正常情况下该 channel 应转 20。
...
src_endpoint 非空
dst_endpoint 非空
channel_type 属于 10 支持范围
不是 pad-related
不是 clock-related
不是 feedthrough
不是 exception path
timing_model 非空，且说明该 channel 需要 10 budget
...
- channel 标记为 pad-related 却进入 10。
- channel 标记为 clock-related 却进入 10。
- channel 标记为 feedthrough 却进入 10。
- channel 标记为 exception path 却进入 10。
...
- `timing_model = visible_netlist` 且 `budget_required != yes`，却要求生成 10 budget。
...
- channel 两端 clock 在当前 03 assembled view 中被声明 async / logically exclusive / physically exclusive，但仍生成普通 sync interface budget，且没有 `relationship_override_basis`。
...
- harden SDC 中存在非 pad input/output delay，但集成表单没有对应 channel。
- 集成表单有 channel，但两端 harden SDC 都没有 interface budget。
...
该约束也不进 10，应归 04 作为 SoC IO/pad environment 处理。
...
如果该路径是 configuration handshake，架构要求不按普通同步接口 budget 分析，则不进 10，归 20。
